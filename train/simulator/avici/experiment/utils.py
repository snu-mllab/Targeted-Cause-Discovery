...
def to_dict(d):
    if hasattr(d, "func"):
        ...
        args.update({
            ...
            if (v.default is not inspect.Parameter.empty and k not in args)
        })
...
def load_data(path, *, observ_only, load_heldout=False):
    ...
    # load g
    loaded_data["g"] = onp.array(pd.read_csv(path / FILE_DATA_G, index_col=False, header=None),
                                 dtype=onp.int32)
    ...
    # try to load x_int
    if (p_vals := path / FILE_DATA_X_INT).is_file() and (p_info :=
                                                         path / FILE_DATA_X_INT_INFO).is_file():
    ...
    if load_heldout:
        if (p_ho := path / FILE_DATA_X_OBS_HELDOUT).is_file():
            x_obs_ho_vals = onp.array(pd.read_csv(p_ho, index_col=False, header=None),
                                      dtype=onp.float32)
            loaded_data["x_heldout_obs"] = onp.stack(
                [x_obs_ho_vals, onp.zeros_like(x_obs_ho_vals)], axis=-1)
        if (p_ho_vals := path / FILE_DATA_X_INT_HELDOUT).is_file() and (
                p_ho_info := path / FILE_DATA_X_INT_INFO_HELDOUT).is_file():
            x_int_ho_vals = onp.array(pd.read_csv(p_ho_vals, index_col=False, header=None),
                                      dtype=onp.float32)
            x_int_ho_info = onp.array(pd.read_csv(p_ho_info, index_col=False, header=None),
                                      dtype=onp.float32)
    ...
    if meta_info["model"] == "real":
        if observ_only:
            loaded_data["x_obs"] = onp.concatenate([loaded_data["x_obs"], loaded_data["x_int"]],
                                                   axis=0)
    ...
    # i.e. if we use interventional data, use less observational data instead
        ...
        if observ_only:
            ...
            n_observ_remaining = max(0,
                                     loaded_data["x_obs"].shape[0] - loaded_data["x_int"].shape[0])
            ...
            if load_heldout and "x_heldout_obs" in loaded_data and "x_heldout_int" in loaded_data:
                n_observ_ho_remaining = max(
                    0,
                    loaded_data["x_heldout_obs"].shape[0] - loaded_data["x_heldout_int"].shape[0])
