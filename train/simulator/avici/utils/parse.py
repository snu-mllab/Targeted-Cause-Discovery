...
def _parse_functions_in_config(config):
    ...
    if type(config) in [dict, defaultdict]:
        ...
        assert (type(config) in [int, float, bool, str] or
                config is None), f"Unknown yaml entry `{config}`"
        new = config
...
def load_data_config(path, abspath=False, verbose=True):
    ...
    for descr, config_testcases in spec["data"].items():
        sufficient_observ_data = [
            arg["n_observations_obs"] >= 2 * arg["n_observations_int"] for arg in config_testcases
        ]
        # if not all(sufficient_observ_data):
        #     warnings.warn(
        #         "Not enough observational data sampled when evaluating. Sample 2x more observational "
        ...
        for method, hparams in config.items():
            ...
            if not all_options:
                ...
                for option in all_options:
                    match = list(
                        filter(lambda setting: all([setting[k] == v for k, v in option.items()]),
                               all_full_settings))
                    assert len(match) == 1
