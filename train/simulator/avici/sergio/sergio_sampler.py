...
def grn_sergio(
    ...
    if add_outlier_effect or add_lib_size_effect or add_dropout_effect:
        ...
        assert tech_noise_config in config.keys(
        ), f"tech_noise_config `{tech_noise_config}` not in config keys: `{list(config.keys())}`"
    ...
    for i, ko_target in enumerate(ko_targets):

        c = datetime.now().strftime('%H:%M:%S')
        print(f"[{c}] Sampling data for ko_target {ko_target} ({i+1}/{len(ko_targets)})",
              flush=True)
        ...
        data[data_type]["x_clean"].append(onp.concatenate(expr, axis=1).T)
        ...
        data[data_type]["x_outlier"].append(onp.concatenate(expr, axis=1).T)
        ...
        data[data_type]["x_dropout"].append(onp.concatenate(expr, axis=1).T)
    ...
    x_add_obs = {}
    x_add_int = {}
    for key in ["x_clean", "x_outlier", "x_dropout"]:
        x_add_obs[key] = onp.stack(data["obs"][key]).reshape(-1, n_vars, order="F")
        x_add_int[key] = onp.stack(data["int"][key]).reshape(-1, n_vars, order="F")
    ############################
