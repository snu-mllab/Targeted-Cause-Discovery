...
class Sergio:
    ...
    def __init__(self,
        ...
        self.nConvSteps = np.zeros(
            number_bins)  # This holds the number of simulated steps till convergence
    ...
    def set_scIndices_(self):
        ...
        self.scIndices_ = self.rng.integers(low=-self.sampling_state_ * self.nSC_,
                                            high=0,
                                            size=self.nSC_)
            ...
            if g[0].Type == 'MR':
                ...
                for bIdx in range(self.nBins_):
                    ...
                    for interTuple in params:
                        ...
                        rate += np.abs(interTuple[1]) * self.hill_(meanExp, interTuple[3],
                                                                   interTuple[2], interTuple[1] < 0)
        ...
        if (type == 'MR'):
            ...
            for tupleIdx, rIdx in enumerate(regIndices):
                ...
                for colIdx, bIdx in enumerate(binIndices):
                    hillMatrix[tupleIdx,
                               colIdx] = self.hill_(regGene_allBins[bIdx].Conc[currStep],
                                                    params[tupleIdx][3], params[tupleIdx][2],
                                                    params[tupleIdx][1] < 0)
    ...
    def CLE_simulator_(self, level):
        ...
        while sim_set != []:
            ...
            for gi, bin_list in enumerate(sim_set):
                ...
                if self.noiseType_ == 'sp':
                    ...
                    amplitude_p = np.multiply(self.noiseParamsVector_[gID],
                                              np.power(prod_rate, 0.5))
        ...
        # print(nReqSteps, self.scIndices_)
