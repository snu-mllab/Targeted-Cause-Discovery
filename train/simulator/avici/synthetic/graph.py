...
def _module_extraction(rng,
                       source,
                       d,
                       topk=0.2,
                       at_least_n_regulators=1,
                       make_acyclic=True,
        ...
        for i in subnet:
            for j in onp.where(a[i] == 1)[0]:
                if j not in subnet:
                    ...
                    if at_least_n_regulators > 0 and len(
                            regulators.intersection(subnet)) < at_least_n_regulators:
                        if j in regulators:
                            neighbors.add(j)
                    else:
                        neighbors.add(j)
    ...
    assert len(subnet) == d
    assert at_least_n_regulators == 0 or len(
        regulators.intersection(subnet)) >= at_least_n_regulators
...
def scale_free(rng, n_vars, *, edges_per_var, power=1, permute=True):
    """Power-law in-degree"""
    pyrandom.seed(
        rng.bit_generator.state["state"]["state"])  # seed pyrandom based on state of numpy rng
    _ = rng.normal()  # advance rng state by 1
    perm = rng.permutation(n_vars).tolist()
    g = ig.Graph.Barabasi(n=n_vars, m=edges_per_var, directed=True,
...
def scale_free_indirect(rng, n_vars, *, edges_per_var, power=1, permute=True):
    """Power-law in-degree"""
    pyrandom.seed(
        rng.bit_generator.state["state"]["state"])  # seed pyrandom based on state of numpy rng
    _ = rng.normal()  # advance rng state by 1
    perm = rng.permutation(n_vars).tolist()
    g = ig.Graph.Barabasi(n=n_vars, m=edges_per_var, directed=False,
                          power=power).permute_vertices(perm)
...
def sbm(rng, n_vars, *, edges_per_var, n_blocks, damp=0.2, permute=True):
    ...
    blocks = onp.split(rng.permutation(n_vars), splits) if permute else onp.split(
        onp.arange(n_vars), splits)
...
def yeast(rng,
    ...
    if n_vars < g_raw.shape[0]:
        # extract random subnetwork
        g = _module_extraction(rng,
                               g_raw,
                               n_vars,
                               topk=topk,
                               at_least_n_regulators=at_least_n_regulators,
                               make_acyclic=make_acyclic,
                               return_node_indices=False)
        ...
        dmax = g_raw.shape[0]
        g_raw[onp.diag_indices(dmax)] = 0
        g = _break_cycles_randomly(rng, g_raw)

        g_permuted = g
        # onp.save("/storage/janghyun/datasets/causal/train_sergio_syn/yeast_acyclic.npy", g_permuted)
...
def ecoli(rng,
    ...
    raw.loc[raw[2].isnull(), 2] = rng.choice(["+", "-"],
                                             p=[prob_pos, 1 - prob_pos],
                                             size=raw[2].isnull().sum())
    ...
    effect_signs = raw.loc[:, [2]].applymap(lambda x: {
        "+": 1.0,
        "-": -1.0
    }[x]).to_numpy().squeeze(-1)
    ...
    if n_vars < g_raw.shape[0]:
        # extract random subnetwork
        g, subnet = _module_extraction(rng,
                                       g_raw,
                                       n_vars,
                                       topk=topk,
                                       at_least_n_regulators=at_least_n_regulators,
        ...
        g_raw[onp.diag_indices(dmax)] = 0
        g = _break_cycles_randomly(rng, g_raw)
        effect_raw_sgn *= g.astype(onp.float32)  # mask in case a cycle was broken

        g_permuted = g
        effect_sgn_permuted = effect_raw_sgn
